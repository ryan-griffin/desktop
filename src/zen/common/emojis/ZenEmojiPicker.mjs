...
class nsZenEmojiPicker extends nsZenDOMOperatedFeature {
  ...
  #emojiListBuilt = false;
  #svgListBuilt = false;
  #emojiSearchEntries = null;
  ...
  #ensureEmojiSearchEntries() {
    if (this.#emojiSearchEntries) {
      return this.#emojiSearchEntries;
    }

    this.#emojiSearchEntries = this.#emojis
      .map(emoji => ({
        emoji: emoji.emoji,
        order: emoji.order,
        lowercaseTags: emoji.tags.map(tag => tag.toLowerCase()),
        button: null,
      }))
  ...
  #onSearchInput(event) {
    if (this.#panel.hasAttribute("only-svg-icons") || !this.#emojiListBuilt) {
      return;
    }
    ...
    for (const { button, lowercaseTags } of this.#emojiSearchEntries) {
      if (!button) {
        continue;
      }
      button.hidden =
        hasQuery && !lowercaseTags.some(tag => tag.includes(value));
    }
  }

  #resetEmojiSearch() {
    for (const { button } of this.#emojiSearchEntries) {
      button.hidden = false;
    }
  }

  #ensureEmojiListBuilt() {
    if (this.#emojiListBuilt) {
      return;
    }

    const fragment = document.createDocumentFragment();
    for (const entry of this.#ensureEmojiSearchEntries()) {
      const item = document.createXULElement("toolbarbutton");
      item.className = "toolbarbutton-1 zen-emojis-picker-emoji";
      item.setAttribute("label", entry.emoji);
      item.setAttribute("tooltiptext", "");
      item.style.order = entry.order;
      item.addEventListener("command", () => {
        this.#selectEmoji(entry.emoji);
      });
      entry.button = item;
      fragment.appendChild(item);
    }
    this.emojiList.appendChild(fragment);
    this.#emojiListBuilt = true;
  }

  #ensureSvgListBuilt() {
    if (this.#svgListBuilt) {
      return;
    }
    ...
    this.svgList.appendChild(fragment);
    this.#svgListBuilt = true;
  }
  ...
  async #onPopupShowing(event) {
    ...
    if (allowEmojis) {
      this.#ensureEmojiListBuilt();
      this.#resetEmojiSearch();
      setTimeout(() => {
        this.searchInput.focus();
      }, 500);
    }
    this.#ensureSvgListBuilt();
  }
